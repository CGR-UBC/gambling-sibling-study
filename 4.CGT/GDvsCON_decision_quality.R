...
setwd("/Users/eoldfiel/Data/github/SIBSTUDY_slots/4.CGT/analysis/model1_rational/GDvsCON")
...
data<- within (data, {
  group<-factor(group,levels=1:2,labels = c("Controls","Gambling disorder"))
})
...
write.csv(tally_ratio,file ='GDvsCON_rational_model_trials_per_participant.csv',row.names = FALSE)
...
######## logistic regression for rational decision ############
...
confint(model)
#removed non sig interactions as hypothesis is main effects
...
odd_ratios<-exp(tab)
write.csv(tab,file ='GDvsCON_rational_model_betas.csv',row.names = FALSE)
write.csv(odd_ratios,file ='GDvsCON_rational_model_OR.csv',row.names = FALSE)
...
write.csv(rational.plot.data,file ='GDvsCON_rational_model_pp.csv',row.names = FALSE)
