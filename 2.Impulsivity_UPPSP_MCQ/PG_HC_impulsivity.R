...
data<-subset(data_all, participant < 300)
...
# descrptives
by(kirby.longdata$K,list(kirby.longdata$Magnitude,kirby.longdata$Group),stat.desc,basic=TRUE, norm = TRUE)
kirby.longdata$Group=factor(kirby.longdata$Group)
kirby.longdata$participant=factor(kirby.longdata$participant)

PG_HC<-c(1,-1)
contrasts(kirby.longdata$Group)<-cbind(PG_HC)
...
PU<-data.frame(data$Group,data$PU)
by(PU$data.PU,PU$data.Group,stat.desc,basic=TRUE,norm=TRUE)
